        ...
        <aside class="sidenav navbar navbar-vertical navbar-expand-xs border-radius-lg fixed-start ms-2  bg-white my-2" id="sidenav-main">
            ...
            <div class="collapse navbar-collapse  w-auto " id="sidenav-collapse-main">
                <ul class="navbar-nav">
                    ...
                    <li class="nav-item">
                        <a class="nav-link active bg-gradient-dark text-white" href="managementdiscountcode">
        ...
        <main class="main-content position-relative max-height-vh-100 h-100 border-radius-lg ">
                ...
                <div class="container-fluid py-1 px-3">
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb bg-transparent mb-0 pb-0 pt-1 px-0 me-sm-6 me-5">
                            ...
                            <li class="breadcrumb-item text-sm text-dark active" aria-current="page"><a class="opacity-5 text-dark" href="managementdiscountcode">Quản lý khuyến mãi</a></li>
                        </ol>
                    </nav>
            ...
            <div class="container-fluid py-2">
                <div class="row">
                        ...
                        <div class="card my-4">
                            <div class="card-header p-0 position-relative mt-n4 mx-3 z-index-2">
                                <div class="bg-gradient-dark shadow-dark border-radius-lg pt-4 pb-3 d-flex align-items-center justify-content-between">
                                    <h6 class="text-white text-capitalize ps-3">Mã khuyến mại</h6>
                                    <c:if test="${sessionScope.account.isAdmin == 1}">
                                        <a class="nav-link text-white me-3" href="#" data-bs-toggle="modal" data-bs-target="#createDiscount">
                            ...
                            <div class="card-body px-0 pb-2">
                                <div class="table-responsive">
                                    <c:set var="currentUserIsAdmin" value="${account.isAdmin}" />
                                    <table class="table align-items-center mb-0">
                                        <thead>
                                            <tr>
                                                <th class="text-center text-uppercase text-secondary text-xxs font-weight-bolder opacity-20">Code ID</th>
                                                <th class="text-center text-uppercase text-secondary text-xxs font-weight-bolder opacity-20 ps-1">Code</th>
                                                <th class="text-center text-uppercase text-secondary text-xxs font-weight-bolder opacity-20">Phần trăm giảm</th>
                                                <th class="text-center text-uppercase text-secondary text-xxs font-weight-bolder opacity-20">Ngày hết hạn</th>
                                                <th class="text-center text-uppercase text-secondary text-xxs font-weight-bolder opacity-20">Số lượng</th>
                                                <th class="text-center text-uppercase text-secondary text-xxs font-weight-bolder opacity-20">Khả dụng</th>
                                                    <c:if test="${currentUserIsAdmin == 1}">
                                                    <th class="text-center text-uppercase text-secondary text-xxs font-weight-bolder opacity-20">Chỉnh sửa</th>
                                                    <th class="text-center text-uppercase text-secondary text-xxs font-weight-bolder opacity-20">Xoá</th>
                                                    </c:if>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            <c:forEach items="${pagedDiscountCodes}" var="discountCode">
                                                <tr>
                                                    <td>
                                                        <div class="align-middle text-center text-sm">
                                                            <h6 class="mb-0 text-sm">${discountCode.codeID}</h6>
                                                        </div>
                                                    </td>
                                                    <td class="align-middle text-center text-sm">
                                                        <span class="text-xs font-weight-bold">${discountCode.code}</span>
                                                    </td>
                                                    <td class="align-middle text-center text-sm">
                                                        <span class="text-xs font-weight-bold">${discountCode.discountPercentage}</span>
                                                    </td>
                                                    <td class="align-middle text-center text-sm">
                                                        <span class="text-xs font-weight-bold">${discountCode.expiryDate}</span>
                                                    </td>
                                                    <td class="align-middle text-center text-sm">
                                                        <span class="text-xs font-weight-bold">${discountCode.amount}</span>
                                                    </td>
                                                    <td class="align-middle text-center text-sm">
                                                        <c:choose>
                                                            <c:when test="${discountCode.isActive}">
                                                                <span class="badge badge-sm bg-gradient-success">Khả dụng</span>
                                                            </c:when>
                                                            <c:otherwise>
                                                                <span class="badge badge-sm bg-gradient-danger">Không khả dụng</span>
                                                            </c:otherwise>
                                                        </c:choose>
                                                    </td>
                                                    <c:if test="${currentUserIsAdmin == 1 && discountCode.codeID != null && discountCode.codeID != 0}">
                                                        <td class="align-middle text-center text-sm">
                                                            <a class="badge badge-sm bg-gradient-info" href="editdiscountcode?code=${discountCode.code}">Sửa</a>
                                                        </td>
                                                        <td class="align-middle text-center text-sm">
                                                            <a class="badge badge-sm bg-gradient-danger" href="deletediscountcode?code=${discountCode.code}">Xoá</a>
                                                        </td>
                                                    </c:if>
                                                </tr>
                                            </c:forEach>
                    ...
                    <div class="modal fade" id="createDiscount" tabindex="-1" aria-labelledby="editDiscount" aria-hidden="true">
                        <div class="modal-dialog modal-lg">
                            <div class="modal-content">
                                <div class="modal-header">
                                    <h5 class="modal-title" id="orderEntryLabel">Nhập Sản Phẩm</h5>
                                    <button type="button" class="btn-close btn-danger" data-bs-dismiss="modal" aria-label="Close"></button>
                                </div>
                                <div class="modal-body container-fluid py-2">
                                    <c:set var="status" value="${param.status}" />
                                    <form action="managementdiscountcode" method="POST">
                                        <div class="row">
                                            <div class="col-md-6 mb-3">
                                                <label for="name" class="form-label"><strong>CODE</strong></label>
                                                <input type="text" class="form-control" name="name" id="name" required>
                                                </div>
                                                <div class="col-md-6 mb-3">
                                                    <label for="amount" class="form-label"><strong>Phần trăm giảm</strong></label>
                                                    <input type="number" class="form-control" name="amount" id="amount" min="1" required>
                                                </div>
                                                <div class="col-md-6 mb-3">
                                                    <label for="expiry" class="form-label"><strong>Ngày hết hạn</strong></label>
                                            ...
                                            <div class="col-md-6 mb-3">
                                                <label for="status" class="form-label"><strong>Bật/tắt mã giảm giá</strong></label>
                                                <select class="form-control" name="status" id="status" required>
                                                    <option value="active" ${status == 'active' ? 'selected' : ''}>Bật</option>
                                                    <option value="inactive" ${status == 'inactive' ? 'selected' : ''}>Tắt</option>
                                                </select>
                                            </div>
                                        </div>
                                        <div class="d-flex justify-content-end mt-4">
                                            <button type="submit" class="btn btn-info me-2" name="action" value="update">Xác nhận</button>
